...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        IDictionary<IterableString, Integer> temp = new ChainingHashDictionary<>(MoveToFrontDictionary::new);
        if(map.get(ngram) == null){
            return null;
        }
        System.out.println(map.get(ngram).keySet());
        for(IterableString s: map.get(ngram).keySet()){
        ...
        IPriorityQueue.PQElement<String>[] ret = new IPriorityQueue.PQElement[temp.size()];
        int c = 0;
        System.out.println(temp.values());
        System.out.println(temp.keySet());
//        for(String s: temp.keySet()){
//            ret[c] = new IPriorityQueue.PQElement<>(s, temp.get(s));
//            c++;
//        }
