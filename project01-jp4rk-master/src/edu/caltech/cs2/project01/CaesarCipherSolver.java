...
public class CaesarCipherSolver {
    ...
    public static List<String> splitBySpaces(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == ' ') {
                lst.add(s.substring(ans, i));
                ans = i + 1;
            }
        }
        lst.add(s.substring(ans, s.length()));
        return lst;
    }
    ...
    public static void main(String[] args) {
        ...
        List<String> leest = new ArrayList<>();
        leest = splitBySpaces(input);
        String ans = "";
        for (int i = 0; i < 26; i++) {
            rot(leest, i);
            if ((howManyWordsIn(leest, dictionary)) > (leest.size() / 2)) {
                ans = putTogetherWithSpaces(leest);
